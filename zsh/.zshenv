...
if [ -t 1 ]; then
    ...
    # Helpful aliases
    if [[ -x "$(which eza)" ]]; then
        alias l='eza -lh --icons=auto --git-repos --no-time --no-user' \
            ll='eza -lha --icons=auto --sort=name --group-directories-first --git-repos --no-time --no-user' \
            ld='eza -lhD --icons=auto --git-repos --no-time --no-user' \
            lt='eza --icons=auto --tree --git-repos --no-time --no-user'
    fi
